...
contract TestMiniMeCloning {
  ...
  function testCanClone() {
    clone1 = MiniMeToken(token.createCloneToken("ESCBCoin2", 18, "ESCBCoin2", block.number, true));
  ...
  function testCanCloneAfterTransfer() {
    clone2 = MiniMeToken(token.createCloneToken("ESCBCoin2", 18, "ESCBCoin2", block.number, true));
  ...
  function testRecurringClones() {
    ...
    for (uint i = 0; i < 10; i++) {
      lastClone = MiniMeToken(lastClone.createCloneToken("ESCBCoinn", 18, "ESCBCoinn", block.number, true));
    }
