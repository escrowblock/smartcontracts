...
var ESCBTokenSaleTokenMock = artifacts.require("ESCBTokenSaleTokenMock");
...
contract('MiniMeIrrVesDivToken', function(accounts) {
  ...
  beforeEach(async () => {
    const sale = await ESCBTokenSaleTokenMock.new(granter, 1);
  ...
  it('cannot create token grants after losing whitelisting ability', async () => {
    ...
    try {
      await token.grantVestedTokens(receiver, tokenAmount, now, now + 10000, now + 20000, true, false, { from: granter })
    } catch(error) {
  ...
  describe('getting a token grant', async () => {
    ...
    it('can handle composed vesting schedules', async () => {
      ...
      await token.grantVestedTokens(receiver, tokenAmount, newNow, newNow + cliff, newNow + vesting, true, false, { from: granter });
